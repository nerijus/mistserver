...
var UI = {
  ...
  interval: {
    ...
    set: function (callback, delay) {
      if (this.opts) {
        log(
          "[interval]",
          "Set called on interval, but an interval is already active."
        );
      }
    ...
    function createButton(j, button) {
      ...
      if ("link" in button) {
        ...
        $button.click(function (e) {
          if ($(this).closest(".menu").hasClass("hide")) {
            return;
          }
          UI.navto(j);
          e.stopPropagation();
    ...
    for (var i in elements) {
      ...
      if (e.type == "help") {
        var $s = $("<span>")
          ...
          .append($("<span>").addClass("description").append(e.help));
        $c.append($s);
        if ("classes" in e) {
      ...
      if (e.type == "text") {
        $c.append(
          $("<span>")
            .addClass("text_container")
            .append($("<span>").addClass("text").append(e.text))
        );
        continue;
      }
      if (e.type == "custom") {
          ...
          switch (button.type) {
            ...
            case "save":
              $b.addClass("save").click(function (e) {
                ...
                $ic
                  .find(
                    '.hasValidate:visible, input[type="hidden"].hasValidate'
                  )
                  .each(function () {
                ...
                //for all inputs
                $ic
                  .find('.isSetting:visible, input[type="hidden"].isSetting')
                  .each(function () {
                    var val = $(this).getval();
      ...
      switch (e.type) {
        ...
        case "radioselect":
          ...
          for (var i in e.radioselect) {
            ...
            var $label = $("<label>")
              .append($radio)
              .append($("<span>").html(e.radioselect[i][1]));
            $field.append($label);
            if (e.radioselect[i].length > 2) {
              ...
              for (var j in e.radioselect[i][2]) {
                ...
                if (e.radioselect[i][2][j] instanceof Array) {
                  $option
                    .val(e.radioselect[i][2][j][0])
                    .html(e.radioselect[i][2][j][1]);
                } else {
        ...
        case "checklist":
          ...
          for (var i in e.checklist) {
            ...
            $checklist.append(
              $("<label>")
                .text(e.checklist[i][1])
                .prepend(
                  $("<input>")
                    .attr("type", "checkbox")
                    .attr("name", e.checklist[i][0])
                )
            );
          }
          break;
            ...
            if (typeof e.selectinput[i] == "string") {
              ...
              if (typeof e.selectinput[i][0] == "string") {
                ...
                if (!$select.data("input")) {
                  $select.data(
                    "input",
                    UI.buildUI([e.selectinput[i][0]]).children()
                  );
                }
              }
            }
        ...
        case "sublist": {
          ...
          $field.data("build", function (values, index) {
            ...
            var newUI = UI.buildUI(
              [$("<h4>").text(mode + " " + local_e.itemLabel)]
                .concat(sublist)
                .concat([
                  {
                    label: "Save first",
          ...
          sublist.unshift({
            ...
            placeholder: "none",
            help:
              "A convenient name to describe this " +
              e.itemLabel +
              ". It won't be used by MistServer.",
            pointer: {
              main: e.saveas,
              index: "x-LSP-name",
            },
      ...
      if ("pointer" in e) {
        $field.attr("name", e.pointer.index);
      }
      $fc.append($field);
      if ("classes" in e) {
        for (var j in e.classes) {
      ...
      if ("unit" in e) {
        $fc.append($("<span>").addClass("unit").html(e.unit));
      }
      if ("readonly" in e) {
          ...
          $("<span>")
            ...
            .html(
              $("<button>")
                ...
                .click(function () {
                  var text = String(
                    $(this).closest(".field_container").find(".field").getval()
                  );
                  var $qr = $("<div>").addClass("qrcode");
                  UI.popup.show(
                    $("<span>")
                      .addClass("qr_container")
                      .append($("<p>").text(text))
                      .append($qr)
                  );
                  $qr.qrcode({
      ...
      switch (e.type) {
          ...
          $browse_button.click(function () {
            ...
            $bc
              .append(
                $("<label>")
                  .addClass("UIelement")
                  .append($("<span>").addClass("label").text("Current folder:"))
                  .append(
                    $("<span>")
                  ...
                  if (d.browse.subdirectories) {
                    ...
                    for (var i in d.browse.subdirectories) {
                      ...
                      $folder_contents.append(
                        $folder
                          .clone(true)
                          .attr("title", $path.text() + seperator + f)
                          .text(f)
                      );
                    }
                  }
        ...
        case "hostlimited":
          ...
          subUI.blackwhite = $("<select>")
            .append($("<option>").val("-").text("Blacklist"))
            .append($("<option>").val("+").text("Whitelist"));
          ...
          switch (e.type) {
            case "geolimited":
              ...
              break;
            case "hostlimited":
              subUI.prototype = $("<input>")
                .attr("type", "text")
                .attr("placeholder", "type a host");
              break;
          }
      ...
      if ("datalist" in e) {
        ...
        for (var i in e.datalist) {
          $datalist.append($("<option>").val(e.datalist[i]));
        }
      }
      ...
      $e.append($ihc);
      if ("help" in e) {
        $ihc.append($("<span>").addClass("ih_balloon").html(e.help));
        $field
      ...
      if ("validate" in e) {
        ...
        $field
          .data("validate_functions", fs)
          .data("help_container", $ihc)
          .data("validate", function (me, focusonerror) {
            if (!$(me).is(":visible") && !$(me).is('input[type="hidden"]')) {
              return;
            }
            ...
            for (var i in fs) {
              ...
              if (error) {
                ...
                return typeof error == "object" && "break" in error
                  ? error["break"]
                  : true;
              }
            }
  ...
  buildVheaderTable: function (opts) {
    ...
    var $header = $("<tr>")
      ...
      .append(
        $("<td>")
          .addClass("vheader")
          .attr("rowspan", opts.labels.length + 1)
          .append($("<span>").text(opts.vheader))
      );
    var $trs = [];
    $header.append($("<td>"));
    ...
    for (var j in opts.content) {
      $header.append($("<td>").html(opts.content[j].header));
      for (var i in opts.content[j].body) {
        $trs[i].append($("<td>").html(opts.content[j].body[i]));
      }
    }

    //no thead, it messes up the vheader
    $table.append($("<tbody>").append($header).append($trs));
  ...
  plot: {
    addGraph: function (saveas, $graph_c) {
      ...
      graph.elements.cont
        .append(graph.elements.plot)
        .append(graph.elements.legend);
    ...
    go: function (graphs) {
        ...
        for (var d in graphs[g].datasets) {
          ...
          switch (set.datatype) {
            ...
            case "perc_retrans":
              switch (set.origin[0]) {
                case "total":
                  reqobj["totals"].push({ fields: [set.datatype], end: -15 });
                  break;
                case "stream":
                  reqobj["totals"].push({
                    fields: [set.datatype],
                    streams: [set.origin[1]],
                    end: -15,
                  });
                  break;
                case "protocol":
      ...
      if (reqobj.totals.length == 0) {
        delete reqobj.totals;
      }
      if (reqobj.clients.length == 0) {
        delete reqobj.clients;
      }
        ...
        for (var g in graphs) {
          ...
          switch (graph.xaxis) {
            case "time":
              ...
              graph.plot = $.plot(graph.elements.plot, plotsets, {
                legend: { show: false },
                ...
                grid: {
                  ...
                  color: "black",
                  backgroundColor: {
                    colors: ["rgba(0,0,0,0)", "rgba(0,0,0,0.025)"],
                  },
                },
                crosshair: {
                  mode: "x",
              ...
              var $list = $("<table>")
                ...
                .html(
                  $("<tr>")
                    .html($("<td>").html($("<h3>").text(graph.id)))
                    .append(
                      $("<td>")
                        .css("padding-right", "2em")
                        .css("text-align", "right")
                        .html($("<span>").addClass("value"))
                        .append(
                          $("<button>")
                            .data("opts", graph)
                            .text("X")
                            ...
                            .click(function () {
                              ...
                              if (
                                ...
                                var $opt = $(
                                  ".graph_ids option:contains(" + graph.id + ")"
                                );
              ...
              function updateLegendValues(x) {
                ...
                for (var i in graph.datasets) {
                  ...
                  if (graph.datasets[i].display) {
                    var tickformatter =
                      UI.plot.yaxes[graph.datasets[i].yaxistype].tickFormatter;
                    var data = graph.datasets[i].data;
                    if (!x) {
                      label = tickformatter(
                        graph.datasets[i].data[
                          graph.datasets[i].data.length - 1
                        ][1]
                      );
                    } else {
                      for (var j in data) {
                        ...
                        $("<label>")
                          ...
                          .append(
                            $("<div>")
                              ...
                              .css("background-color", graph.datasets[i].color)
                          )
                          .append(graph.datasets[i].label)
                      )
                    )
                    .append(
                      $("<td>")
                        ...
                        $("<table>")
                          ...
                          .append(
                            $("<tr>")
                              ...
                              .append(
                                $("<td>").html(
                                  item.series.yaxis.tickFormatter(
                                    item.datapoint[1],
                                    item.series.yaxis
                                  )
                                )
    ...
    datatype: {
      getOptions: function (opts) {
        ...
        var specialized = $.extend(
          true,
          {},
          UI.plot.datatype.templates[opts.datatype]
        );
      ...
      templates: {
        general: {
          ...
          getdata: function () {
            var streamindex =
              this.origin[0] == "stream" ? this.origin[1] : "all_streams";
            var protocolindex =
              this.origin[0] == "protocol" ? this.origin[1] : "all_protocols";
            var thedata =
              mist.data.totals[streamindex][protocolindex][this.datatype];
            this.data = thedata;
            return thedata;
          },
        },
        cpuload: {
          ...
          getdata: function (dataobj) {
            ...
            this.data.push([
              mist.data.config.time * 1000,
              mist.data.capabilities.cpu_use / 10,
            ]);
            return this.data;
          },
        },
        memload: {
          ...
          getdata: function () {
            ...
            this.data.push([
              mist.data.config.time * 1000,
              mist.data.capabilities.load.memory,
            ]);
            return this.data;
          },
        },
  ...
  format: {
    time: function (secs, type) {
      ...
      if (type != "short") {
        str.push(("0" + d.getSeconds()).slice(-2));
      }
      return str.join(":");
    },
    ...
    duration: function (seconds) {
      ...
      switch (unit) {
        case "year":
          $s.append(UI.format.addUnit(amounts.year, "years, ")).append(
            UI.format.addUnit(amounts.week, "wks")
          );
          break;
        case "week":
          $s.append(UI.format.addUnit(amounts.week, "wks, ")).append(
            UI.format.addUnit(amounts.day, "days")
          );
          break;
        case "day":
          $s.append(UI.format.addUnit(amounts.day, "days, ")).append(
            UI.format.addUnit(amounts.hr, "hrs")
          );
          break;
        default:
          $s.append(
            [
              ...
              ("0" + amounts.sec).slice(-2) +
                (amounts.ms ? "." + amounts.ms : ""),
            ].join(":")
          );
          break;
      }
    ...
    addUnit: function (number, unit) {
      ...
      $s.append($("<span>").addClass("unit").html(unit));
      return $s[0].innerHTML;
    },
    bytes: function (val, persec) {
      ...
      return UI.format.addUnit(
        UI.format.number(val),
        unit + (persec ? "/s" : "")
      );
    },
  },
  navto: function (tab, other) {
    ...
    if (typeof screenlog != "undefined") {
      screenlog.navto(hash[1]);
    } //allow logging if screenlog is active
  ...
  showTab: function (tab, other) {
    ...
    var $currbut = UI.elements.menu
      .removeClass("hide")
      .find('.plain:contains("' + tab + '")')
      .closest(".button");
    if ($currbut.length > 0) {
    ...
    switch (tab) {
      ...
      case "Create a new account":
        ...
        $main.append(
          $("<p>")
            .text("No account has been created yet in the MistServer at ")
            .append($("<i>").text(mist.user.host))
            .append(".")
        );
      ...
      case "Account created":
        UI.elements.menu.addClass("hide");
        $main
          .append($("<p>").text("Your account has been created succesfully."))
          .append(
            UI.buildUI([
              {
                type: "text",
                ...
                buttons: [
                  {
                    ...
                    function: function () {
                      if (mist.data.config.protocols) {
                        $main.append(
                          "Unable to enable all protocols as protocol settings already exist.<br>"
                        );
                        return;
                      }
                        ...
                        function (d) {
                          ...
                          for (var i in d.capabilities.connectors) {
                            ...
                            if (connector.required) {
                              $main.append(
                                'Could not enable protocol "' +
                                  i +
                                  '" because it has required settings.<br>'
                              );
                              continue;
                            }

                            protocols.push({ connector: i });
      ...
      case "Overview":
        ...
        var s = {
          ...
          location:
            "location" in mist.data.config ? mist.data.config.location : {},
        };
        var b = {};
        ...
        var $bitunit = $("<select>")
          .html($("<option>").val(1).text("bytes/s"))
          .append($("<option>").val(1024).text("KiB/s"))
          .append($("<option>").val(1048576).text("MiB/s"))
          .append($("<option>").val(1073741824).text("GiB/s"));
        ...
        $main.append(
          UI.buildUI([
            ...
            },
            {
              ...
              value: mist.data.streams
                ? Object.keys(mist.data.streams).length
                : 0,
            },
            {
              type: "span",
            ...
            },
            {
              ...
              help:
                "Make stats available in Prometheus format. These can be accessed via " +
                host +
                "/PASSPHRASE or " +
                host +
                "/PASSPHRASE.json.",
        ...
        if (mist.data.LTS) {
          function update_update(d) {
            ...
            function add_logs(log) {
              var msgs = log.filter(function (a) {
                return a[1] == "UPDR";
              });
              if (msgs.length) {
                var $cont = $("<div>");
            ...
            if (!d.update || !("uptodate" in d.update)) {
              ...
              $versioncheck.append(
                $("<span>")
                  ...
                  .text(
                    "On " +
                      new Date(d.update.date).toLocaleDateString() +
                      " version " +
                      d.update.version +
                      " became available."
                  )
              ...
              if (!d.update.url || d.update.url.slice(-4) != ".zip") {
                ...
                $versioncheck.append(
                  $("<button>")
                    .text("Rolling update")
                    .css({ "font-size": "1em", "margin-left": "1em" })
                    .click(function () {
                      if (
                        confirm(
                          "Are you sure you want to execute a rolling update?"
                        )
                      ) {
                        $versioncheck
                          .addClass("orange")
                          .removeClass("red")
                          .text("Rolling update command sent..");
              ...
              var a = $("<a>")
                .attr("href", d.update.url)
                .attr("target", "_blank")
                .text("Manual download");
              a[0].protocol = "https:";
              $versioncheck.append($("<div>").append(a));
            }
            add_logs(d.log);
          }
          ...
          if ("license" in mist.data.config) {
            if (
              "active_products" in mist.data.config.license &&
              Object.keys(mist.data.config.license.active_products).length
            ) {
              var $t = $("<table>").css("text-indent", "0");
              $activeproducts.html($t);
              $t.append(
                $("<tr>")
                  .append($("<th>").append("Product"))
                  .append($("<th>").append("Updates until"))
                  .append($("<th>").append("Use until"))
                  .append($("<th>").append("Max. simul. instances"))
              );
              for (var i in mist.data.config.license.active_products) {
                var p = mist.data.config.license.active_products[i];
                $t.append(
                  $("<tr>")
                    .append($("<td>").append(p.name))
                    .append(
                      $("<td>").append(
                        p.updates_final ? p.updates_final : "&infin;"
                      )
                    )
                    .append(
                      $("<td>").append(p.use_final ? p.use_final : "&infin;")
                    )
                    .append($("<td>").append(p.amount ? p.amount : "&infin;"))
                );
              }
            } else {
            ...
            $activeproducts.append(
              $("<a>")
                .text("More details")
                .attr("href", "https://shop.mistserver.org/myinvoices")
                .attr("target", "_blank")
            );
          }
        } else {
        ...
        function enterStats() {
          ...
          if ("totals" in mist.data && "all_streams" in mist.data.totals) {
            ...
            clients = clients.length
              ? UI.format.number(clients[clients.length - 1][1])
              : 0;
          } else {
            clients = "Loading..";
          }
          ...
          if (
            "license" in mist.data.config &&
            "user_msg" in mist.data.config.license
          ) {
            mist.data.log.unshift([
              mist.data.config.license.time,
              "ERROR",
              mist.data.config.license.user_msg,
            ]);
          }
          for (var i in mist.data.log) {
            ...
            if (["FAIL", "ERROR"].indexOf(l[1]) > -1) {
              ...
              var split = l[2].split("|");
              for (var i in split) {
                $content.append($("<span>").text(split[i]));
              }
              $errors.append(
                $("<div>")
                  .append($("<span>").append(UI.format.time(l[0])))
                  .append($content)
              );
              if (n == 5) {
                break;
          ...
          for (var i in mist.data.config.protocols) {
            ...
            if (protocols.on.indexOf(p.connector) > -1) {
              continue;
            }
            protocols.on.push(p.connector);
          }
          $protocols_on.text(
            protocols.on.length ? protocols.on.join(", ") : "None."
          );
          if ("capabilities" in mist.data) {
            ...
            $protocols_off.text(
              protocols.off.length ? protocols.off.join(", ") : "None."
            );
          } else {
      ...
      case "Protocols":
        ...
        $main
          ...
          .append(
            $("<button>")
              .text("Delete all protocols")
              .click(function () {
                if (
                  confirm(
                    "Are you sure you want to delete all currently configured protocols?"
                  )
                ) {
              ...
              .click(function () {
                ...
                for (var i in toenable) {
                  if (
                    !(
                      "required" in
                      mist.data.capabilities.connectors[toenable[i]]
                    ) ||
                    Object.keys(
                      mist.data.capabilities.connectors[toenable[i]].required
                    ).length == 0
                  ) {
                ...
                var msg =
                  "Click OK to enable disabled protocols with their default settings:" +
                  "\n  ";
                if (dontskip.length) {
                  msg += dontskip.join(", ");
                } else {
                ...
                if (confirm(msg) && dontskip.length) {
                  if (mist.data.config.protocols === null) {
                    mist.data.config.protocols = [];
                  }
          ...
          .append(
            $("<table>")
              .html(
                $("<thead>").html(
                  $("<tr>")
                    .html($("<th>").text("Protocol"))
                    .append($("<th>").text("Status"))
                    .append($("<th>").text("Settings"))
                    .append($("<th>"))
                )
              )
              .append($tbody)
        ...
        function updateProtocols() {
          function displaySettings(protocol) {
            var capabilities =
              mist.data.capabilities.connectors[protocol.connector];
            if (!capabilities) {
              return "";
            }
            var str = [];
          ...
          for (var i in mist.data.config.protocols) {
            ...
            $tbody.append(
              $("<tr>")
                ...
                .append(
                  $("<td>").text(
                    capa && capa.friendly ? capa.friendly : protocol.connector
                  )
                )
                .append($("<td>").html(UI.format.status(protocol)))
                .append($("<td>").html(displaySettings(protocol)))
                .append(
                  $("<td>")
                    .css("text-align", "right")
                    .html(
                          ...
                          if (
                            confirm(
                              'Are you sure you want to delete the protocol "' +
                                mist.data.config.protocols[index].connector +
                                '"?'
                            )
                          ) {
                            mist.send(
                              ...
                              {
                                deleteprotocol:
                                  mist.data.config.protocols[index],
                              }
                            );
      ...
      case "Edit Protocol":
        ...
        function buildProtocolSettings(kind) {
          ...
          if ("deps" in input && input.deps != "") {
            ...
            $ul = $("<ul>");
            $t.append($ul);
            if (typeof input.deps == "string") {
              input.deps = input.deps.split(", ");
            }
            for (var i in input.deps) {
              var $li = $("<li>").text(input.deps[i] + " ");
              $ul.append($li);
              if (
                typeof current[input.deps[i]] != "undefined" ||
                typeof current[input.deps[i] + ".exe"] != "undefined"
              ) {
                //also check for the windows executable
                $li.append($("<span>").addClass("green").text("(Configured)"));
              } else {
                $li.append(
        ...
        if (!editing) {
          //new
          $main.html($("<h2>").text("New Protocol"));
          var saveas = {};
          var select = [["", ""]];
          for (var i in mist.data.capabilities.connectors) {
            select.push([
              i,
              mist.data.capabilities.connectors[i].friendly
                ? mist.data.capabilities.connectors[i].friendly
                : i,
            ]);
          }
      ...
      case "Streams":
        ...
        $switchmode
          .text(
            "Switch to " +
              (other == "thumbnails" ? "list" : "thumbnail") +
              " view"
          )
          .click(function () {
            mist.stored.set(
              "viewmode",
              other == "thumbnails" ? "list" : "thumbnails"
            );
          ...
          switch (type) {
            case "thumbnails": {
              ...
              function selectastream(select, folders) {
                ...
                select.unshift("");
                $loading.remove();
                $main.append($("<h2>").text(tab)).append(
                  UI.buildUI([
                    {
                ...
                for (var i in select) {
                  ...
                  var $edit = $("<button>")
                    ...
                    .click(function () {
                      UI.navto(
                        "Edit",
                        $(this).closest("div").attr("data-stream")
                      );
                    });
                  var $preview = $("<button>")
                    .text("Preview")
                  ...
                  if (thumbnails && folders.indexOf(streamname) == -1) {
                    ...
                    $image.append(
                      $("<img>")
                        .attr("src", thumbnails(streamname))
                        .error(function () {
                          $(this).hide();
                        })
                    );
                  ...
                  if (streamname.indexOf("+") > -1) {
                    ...
                    source =
                      mist.data.streams[streambits[0]].source + streambits[1];
                    $delete = "";
                    $edit = "";
                  ...
                  $shortcuts.append(
                    $("<div>")
                      ...
                      .append($image)
                      .append($("<span>").addClass("description").text(source))
                      .append(
                        $("<span>")
                          .addClass("button_container")
                          .append($edit)
            ...
            default: {
              var $tbody = $("<tbody>").append(
                $("<tr>").append("<td>").attr("colspan", 6).text("Loading..")
              );
              var $table = $("<table>")
                .html(
                  $("<thead>").html(
              ...
              function buildStreamTable() {
                ...
                for (var s in streams) {
                  ...
                  if (streamname in mist.data.streams) {
                    stream = mist.data.streams[streamname];
                  } else {
                    stream = allstreams[streamname];
                  }
                  ...
                  var v = 0;
                  if (
                    typeof mist.data.totals != "undefined" &&
                    typeof mist.data.totals[streamname] != "undefined"
                  ) {
                    var data =
                      mist.data.totals[streamname].all_protocols.clients;
                    var v = 0;
              ...
              if (mist.data.LTS) {
                ...
                for (var s in mist.data.streams) {
                  var inputs_f =
                    mist.data.capabilities.inputs.Folder ||
                    mist.data.capabilities.inputs["Folder.exe"];
                  if (!inputs_f) {
                    break;
                  }
                  if (
                    mist.inputMatch(
                      inputs_f.source_match,
                      mist.data.streams[s].source
                    )
                  ) {
                    ...
                    mist.send(
                      function (d, opts) {
                        var s = opts.stream;
                        var matches = 0;
                        outer: for (var i in d.browse.files) {
                          inner: for (var j in mist.data.capabilities.inputs) {
                            if (
                              j.indexOf("Buffer") >= 0 ||
                              j.indexOf("Buffer.exe") >= 0 ||
                              j.indexOf("Folder") >= 0 ||
                              j.indexOf("Folder.exe") >= 0
                            ) {
                              continue;
                            }
                            if (
                              mist.inputMatch(
                                mist.data.capabilities.inputs[j].source_match,
                                "/" + d.browse.files[i]
                              )
                            ) {
                              var streamname = s + "+" + d.browse.files[i];
                              allstreams[streamname] = createWcStreamObject(
                                streamname,
                                mist.data.streams[s]
                              );
                              allstreams[streamname].source =
                                mist.data.streams[s].source + d.browse.files[i];
                    ...
                    mist.send(
                      function () {
                        for (var i in mist.data.active_streams) {
                          ...
                          if (
                            split.length > 1 &&
                            split[0] in mist.data.streams
                          ) {
                            select[mist.data.active_streams[i]] = true;
                            allstreams[mist.data.active_streams[i]] =
                              createWcStreamObject(
                                mist.data.active_streams[i],
                                mist.data.streams[split[0]]
                              );
                          }
                        }
      ...
      case "Edit":
        ...
        var $style = $("<style>").text(
          "button.saveandpreview { display: none; }"
        );
        ...
        function updateLiveStreamHint() {
          ...
          for (var i in trythese) {
            if (trythese[i] in mist.data.capabilities.connectors) {
              port[trythese[i]] =
                mist.data.capabilities.connectors[trythese[i]].optional.port[
                  "default"
                ];
            }
          }
          ...
          for (var protocol in port) {
            ...
            if (port[protocol] == defport[protocol]) {
              port[protocol] = "";
            } else {
          ...
          for (var i in port) {
            ...
            switch (i) {
              ...
              case "RTMP.exe":
                ...
                $livestreamhint
                  .find(".field.RTMPkey")
                  .setval(streamname == "" ? "STREAMNAME" : streamname)
                  .closest("label")
                  .show();
        ...
        for (var i in mist.data.capabilities.processes) {
          select.push([
            i,
            mist.data.capabilities.processes[i].hrn
              ? mist.data.capabilities.processes[i].hrn
              : mist.data.capabilities.processes[i].name,
          ]);
        }
        if (select.length) {
              ...
              function: function () {
                ...
                if (type != null) {
                  ...
                  $subtypecont.html(
                    UI.buildUI(
                      UIarr.concat(
                        mist.convertBuildOptions(capabilities, newproc)
                      )
                    )
                  );
                }
          ...
          UI.buildUI([
            ...
            },
            {
              ...
              function: function () {
                ...
                for (var i in mist.data.capabilities.inputs) {
                  if (
                    typeof mist.data.capabilities.inputs[i].source_match ==
                    "undefined"
                  ) {
                    continue;
                  }
                  if (
                    mist.inputMatch(
                      mist.data.capabilities.inputs[i].source_match,
                      source
                    )
                  ) {
                    type = i;
                ...
                if (type === null) {
                  $inputoptions
                    .html($("<h3>").text("Unrecognized input").addClass("red"))
                    .append(
                      $("<span>")
                        .text("Please edit the stream source.")
                        .addClass("red")
                    );
                  return;
                }
                ...
                var build = mist.convertBuildOptions(input, saveas);
                if (
                  "always_match" in mist.data.capabilities.inputs[i] &&
                  mist.inputMatch(
                    mist.data.capabilities.inputs[i].always_match,
                    source
                  )
                ) {
                  build.push({
                ...
                if (input.name == "Folder") {
                  ...
                  ["Buffer", "Buffer.exe", "TS", "TS.exe"].indexOf(input.name) >
                  -1
                ) {
                  var fields = [
                    $("<br>"),
      ...
      case "Preview":
        ...
        $main
          ...
          .append($("<h2>").text('Preview of "' + other + '"'))
          .append($cont);
        ...
        if (!$("link#devcss").length) {
          $main.append(
            $("<link>")
              .attr("rel", "stylesheet")
              .attr("type", "text/css")
              .attr("href", embedbase + "skins/dev.css")
              .attr("id", "devcss")
          );
        }
                      ...
                      },
                      {
                        ...
                        children: [
                          ...
                          },
                          {
                            type: "button",
                            title:
                              "Switch to the next available player and source combination",
        ...
        var $trackinfo = $("<div>").append($("<h3>").text("Meta information"));
        ...
        function buildTrackinfo(info) {
            ...
            switch (track.type) {
              case "audio":
                tables.audio.content.push({
                  ...
                  body: [
                    track.codec,
                    UI.format.duration((track.lastms - track.firstms) / 1000) +
                      "<br><span class=description>" +
                      UI.format.duration(track.firstms / 1000) +
                      " to " +
                      UI.format.duration(track.lastms / 1000) +
                      "</span>",
                    peakoravg(track, "bps"),
              ...
              case "video":
                tables.video.content.push({
                  ...
                  body: [
                    ...
                    peakoravg(track, "bps"),
                    peakoravg(track, "maxbps"),
                    UI.format.addUnit(track.width, "x ") +
                      UI.format.addUnit(track.height, "px"),
                    UI.format.addUnit(
                      UI.format.number(track.fpks / 1000),
                      "fps"
                    ),
                    "language" in track ? track.language : "unknown",
                    trackindex.video,
                ...
                if (track.codec == "subtitle" || track.type == "subtitle") {
                  tables.subtitle.content.push({
                    ...
                    body: [
                      track.codec,
                      UI.format.duration(
                        (track.lastms - track.firstms) / 1000
                      ) +
                        "<br><span class=description>" +
                        UI.format.duration(track.firstms / 1000) +
                        " to " +
          ...
          for (var i in tracktypes) {
            if (tables[tracktypes[i]].content.length) {
              $c.append(
                UI.buildVheaderTable(tables[tracktypes[i]]).css("width", "auto")
              );
            }
          }
        ...
        if (escapedstream != "") {
          ...
          function updateProcessInfo() {
            ...
            if (!mist.data.capabilities) {
              req.capabilities = true;
            }
            mist.send(function (data) {
              if (data.proc_list) {
                var $processes = $("<table>").css("width", "auto");
                var layout = {
                  "Process type:": function (d) {
                    return $("<b>").text(d.process);
                  },
                  "Source:": function (d) {
                        ...
                        $("<span>")
                          ...
                          .text(
                            " track " +
                              d.source_tracks
                                .slice(0, -2)
                                .concat(d.source_tracks.slice(-2).join(" and "))
                                .join(", ")
                          )
                      );
                    }
                        ...
                        $("<span>")
                          ...
                          .text(
                            " track " +
                              d.sink_tracks
                                .slice(0, -2)
                                .concat(d.sink_tracks.slice(-2).join(" and "))
                                .join(", ")
                          )
                      );
                    }
                  ...
                  "Active for:": function (d) {
                    var since = new Date().setSeconds(
                      new Date().getSeconds() - d.active_seconds
                    );
                    return $("<span>")
                      .append(
                      ...
                      .append(
                        $("<span>")
                          .addClass("description")
                          .text(" since " + UI.format.time(since / 1e3))
                      );
                  },
                  "Pid:": function (d, i) {
                    return i;
                  ...
                  "Additional info:": function (d) {
                    ...
                    if (d.ainfo && Object.keys(d.ainfo).length) {
                      ...
                      for (var i in d.ainfo) {
                        var legend =
                          mist.data.capabilities.processes[d.process].ainfo[i];
                        if (!legend) {
                          legend = {
                            name: i,
                          };
                      ...
                      $t = $("<span>").addClass("description").text("N/A");
                    }
                    return $t;
                  },
                };
                $processinfo
                  .html($("<h4>").text("Stream processes"))
                  .append($processes);
                for (var i in layout) {
                  var $tr = $("<tr>");
                  $processes.append($tr);
                  $tr.append($("<th>").text(i).css("vertical-align", "top"));
                  for (var j in data.proc_list) {
                    $out = layout[i](data.proc_list[j], j);
                    $tr.append(
                      $("<td>").html($out).css("vertical-align", "top")
                    );
                  }
                }
              }
      ...
      case "Embed":
        ...
        if (otherhost.host || otherhost.https) {
          otherbase =
            (otherhost.https && https_port ? "https://" : "http://") +
            (otherhost.host ? otherhost.host : parsed.host) +
            (otherhost.https && https_port ? https_port : http_port) +
            "/";
          if (otherhost.host) {
            if (!("http" in puburls)) {
              other_split.http = parseURL(other_split.http, {
                hostname: otherhost.host,
              }).full;
            }
            if ("https" in other_split && !("https" in puburls)) {
              other_split.https = parseURL(other_split.https, {
                hostname: otherhost.host,
              }).full;
            }
          }
        ...
        function embedhtml() {
          ...
          for (var i in embedoptions) {
            if (i == "prioritize_type") {
              if (embedoptions[i] && embedoptions[i] != "") {
                options.push(
                  "forcePriority: " +
                    JSON.stringify({ source: [["type", [embedoptions[i]]]] })
                );
              }
              continue;
            }
            ...
            if (
              embedoptions[i] != defaultembedoptions[i] &&
              embedoptions[i] != null &&
              (typeof embedoptions[i] != "object" ||
                JSON.stringify(embedoptions[i]) !=
                  JSON.stringify(defaultembedoptions[i]))
            ) {
          ...
          output.push(
            '    <a href="' +
              (otherhost.https ? other_split.https : other_split.http) +
              escapedstream +
              ".html" +
              '" target="_blank">'
          );
          ...
          if (
            "https" in embedbase &&
            parseURL(embedbase.http).protocol != "https://"
          ) {
            output.push(
              '      if (location.protocol == "https:") { p.src = "' +
                other_split.https +
                'player.js" } '
            );
            output.push(
              '      else { p.src = "' + other_split.http + 'player.js" } '
            );
          } else {
        ...
        $embedlinks
                    ...
                    $("<span>")
                      ...
                      .append(
                        $("<input>")
                          .attr("type", "text")
                          .addClass("field")
                          .val(otherhost.host ? otherhost.host : parsed.host)
                      )
                      .append(
                        $("<span>")
                          .addClass("unit")
          ...
          .append(
            UI.buildUI([
              ...
              $("<h4>")
                .text("Embed code options (optional)")
                .css("margin-top", 0),
              {
                type: "help",
              ...
              },
              {
                ...
                type: "select",
                select: [
                  ["", "Ask the viewer what to do"],
                  ["nextCombo", "Try the next source / player combination"],
                ],
                pointer: {
                  main: embedoptions,
        ...
        $.ajax({
          ...
          success: function (d) {
            ...
            for (var i in d.source) {
              ...
              var human = UI.humanMime(source.type);
              if (
                $s_forceType.children('option[value="' + source.type + '"]')
                  .length == 0
              ) {
                $s_forceType.append(
                  $("<option>")
            ...
            for (var i in d.meta.tracks) {
              ...
              tracks[t.type].push([
                t.trackid,
                UI.format.capital(t.type) +
                  " track " +
                  (tracks[t.type].length + (t.type == "subtitle" ? 1 : 0)),
              ]);
            }
                ...
                var $select = $("<select>")
                  ...
                  .change(function () {
                    if ($(this).val() == "") {
                      ...
                      embedoptions.setTracks[$(this).attr("data-type")] =
                        $(this).val();
                    }
                    $(".embed_code").setval(embedhtml(embedoptions));
                  });
          ...
          function (d) {
            ...
            function buildTr(push, type) {
              ...
              if (type == "Automatic" && push.length >= 4) {
                $target.append($("<span>").text(push[2]));
                if (push[3]) {
                  $target.append(
                    $("<span>").text(
                      ", schedule on " +
                        new Date(push[3] * 1e3).toLocaleString()
                    )
                  );
                }
                if (push.length >= 5 && push[4]) {
                  $target.append(
                    $("<span>").text(
                      ", complete on " +
                        new Date(push[4] * 1e3).toLocaleString()
                    )
                  );
                }
              } else if (push.length >= 4 && push[2] != push[3]) {
                $target
                  .append($("<span>").text(push[2]))
                  .append(
                    $("<span>")
                      .html("&#187")
                      .addClass("unit")
                  ...
                  .append($("<span>").text(push[3]));
              } else {
                $target.append($("<span>").text(push[2]));
              }
              var $buttons = $("<td>").append(
                $("<button>")
                  .text(type == "Automatic" ? "Remove" : "Stop")
                  .click(function () {
                    if (
                      confirm(
                      ...
                      $tr.html(
                        $("<td colspan=99>").html(
                          $("<span>")
                            .addClass("red")
                            .text(
                              type == "Automatic" ? "Removing.." : "Stopping.."
                            )
                        )
                      );
                      if (type == "Automatic") {
                ...
                $buttons.append(
                  $("<button>")
                    ...
                    .click(function () {
                      if (
                        confirm(
                          'Are you sure you want to stop all pushes matching \n"' +
                            push[1] +
                            " to " +
                            push[2] +
                            '"?' +
                        ...
                        for (var i in d.push_list) {
                          //                streamname                        target
                          if (
                            push[1] == d.push_list[i][1] &&
                            push[2] == d.push_list[i][2]
                          ) {
                            pushIds.push(d.push_list[i][0]);
                            $push
                              .find("tr[data-pushid=" + d.push_list[i][0] + "]")
                              .html(
                                $("<td colspan=99>").html(
                ...
                if (push.length >= 6) {
                  ...
                  $logs
                    .append(
                      $("<div>").append(
                        "Active for: " +
                          UI.format.duration(stats.active_seconds)
                      )
                    )
                    .append(
                      $("<div>").append(
                    ...
                    .append(
                      $("<div>").append(
                        "Media time transfered: " +
                          UI.format.duration(stats.mediatime * 1e-3)
                      )
                    );
                    ...
                    $logs.append(
                      $("<div>").append(
                        "Packets retransmitted: " +
                          UI.format.number(stats.pkt_retrans_count || 0)
                      )
                    );
                  }
                  if ("pkt_loss_count" in stats) {
                    $logs.append(
                      $("<div>").append(
                        "Packets lost: " +
                          UI.format.number(stats.pkt_loss_count || 0) +
                          " (" +
                          UI.format.addUnit(
                            UI.format.number(stats.pkt_loss_perc || 0),
                            "%"
                          ) +
                          " over the last " +
                          UI.format.addUnit(5, "s") +
                          ")"
                      )
                    );
                  }
              ...
              return $("<tr>")
                ...
                .attr("data-pushid", push[0])
                .append($("<td>").text(push[1]))
                .append($("<td>").append($target.children()))
                .append($("<td>").addClass("logs").append($logs.children()))
                .append($buttons);
            }
            ...
            if ($push.find("tr").length == 1) {
              ...
              var streams = [];
              var targets = [];
              var $select_streams = $("<select>")
                .css("margin-left", "0.5em")
                .append($("<option>").text("Any stream").val(""));
              var $select_targets = $("<select>")
                .css("margin-left", "0.5em")
                .append($("<option>").text("Any target").val(""));
              for (var i in d.push_list) {
              ...
              for (var i in streams) {
                $select_streams.append($("<option>").text(streams[i]));
              }
              for (var i in targets) {
                $select_targets.append($("<option>").text(targets[i]));
              }

              $c.append(
                $("<button>")
                  .text("Stop all pushes")
                  .click(function () {
                    var push_list = [];
                    for (var i in d.push_list) {
                ...
                .append(
                  $("<label>")
                    ...
                    .append(
                      $("<span>")
                        .text("Stop all pushes that match: ")
                        .css("font-size", "0.9em")
                    )
                    .append($select_streams)
                    .append(
                      $("<span>")
                        .css("margin-left", "0.5em")
                        .text("and")
                    ...
                    .append(
                      $("<button>")
                        ...
                        .click(function () {
                          ...
                          if (s == "" && t == "") {
                            return alert(
                              "Looks like you want to stop all pushes. Maybe you should use that button?"
                            );
                          }
                          var pushes = {};
                          ...
                          var msg =
                            "Are you sure you want to stop these pushes?" +
                            "\n\n";
                          for (var i in pushes) {
        ...
        function buildTheThings(edit) {
          ...
          var build = [
              ...
              ),
            {
              ...
              type: "str",
              help:
                "Where the stream will be pushed to.<br>\
                  Valid formats:\
                  <ul>\
                    <li>" +
                target_match.join("</li><li>") +
                "</li>\
                  </ul>\
              ...
              validate: [
                ...
                function (val, me) {
                  ...
                  return {
                    msg:
                      "Does not match a valid target.<br>Valid formats:<ul><li>" +
                      target_match.join("</li><li>") +
                      "</li></ul>",
                    classes: ["red"],
                  };
              ...
              function: function () {
                ...
                for (connector in connector2target_match) {
                  for (var i in connector2target_match[connector]) {
                    if (
                      mist.inputMatch(
                        connector2target_match[connector][i],
                        $(this).getval()
                      )
                    ) {
                      match = connector;
                ...
                if (!match) {
                  $additional_params
                    ...
                    .append($("<span>").text("Please edit the push target."));
                  return;
                }
                $additional_params.html(
                  $("<h3>").text(
                    mist.data.capabilities.connectors[match].friendly
                  )
                );
                var capa = {
                  desc: mist.data.capabilities.connectors[match].desc,
                  optional:
                    mist.data.capabilities.connectors[match].push_parameters,
                };
                $additional_params.append(
                  UI.buildUI(mist.convertBuildOptions(capa, saveas.params))
                );
              },
              LTSonly: 1,
            },
            ...
            buttons: [
              ...
              },
              {
                ...
                delete saveas.recstartunix;*/
                  ...
                  var obj = {};
                  obj[other == "auto" ? "push_auto_add" : "push_start"] =
                    saveas;
                  if (
                    typeof edit != "undefined" &&
                    (edit[0] != saveas.stream || edit[1] != saveas.target)
                  ) {
          ...
          mist.send(
            function (d) {
              ...
              for (var i in mist.data.streams) {
                ...
                if (
                  mist.inputMatch(
                    UI.findInput("Folder").source_match,
                    mist.data.streams[i].source
                  )
                ) {
                  ...
                  mist.send(
                    ...
                    { browse: mist.data.streams[i].source },
                    { stream: i }
                  );
                  browserequests++;
                }
                    ...
                    .html(
                      $("<button>")
                        ...
                        .click(function () {
                          UI.navto(
                            "Edit Trigger",
                            $(this).closest("tr").attr("data-index")
                          );
                        })
                    )
                    .append(
                      $("<button>")
                        ...
                        .click(function () {
                          ...
                          if (
                            ...
                            mist.data.config.triggers[index[0]].splice(
                              index[1],
                              1
                            );
                            if (
                              mist.data.config.triggers[index[0]].length == 0
                            ) {
      ...
      case "Edit Trigger":
        ...
        if (!other) {
          //new
          $main.html($("<h2>").text("New Trigger"));
          var saveas = {};
        } else {
          //editing
          other = other.split(",");
          var source = triggerRewrite(
            mist.data.config.triggers[other[0]][other[1]]
          );
          var saveas = {
            triggeron: other[0],
        ...
        $main.append(
          UI.buildUI([
            ...
            },
            {
              ...
              type: "str",
              help: $("<div>")
                .text("The extra data you want this trigger to use.")
                .append($params_help),
              pointer: {
                main: saveas,
                index: "params",
                ...
                },
                {
                  ...
                  function: function () {
                    ...
                    var newtrigger = {
                      ...
                      streams:
                        typeof saveas.appliesto == "undefined"
                          ? []
                          : saveas.appliesto,
                    ...
                    mist.data.config.triggers[saveas.triggeron].push(
                      newtrigger
                    );
      ...
      case "Logs":
        ...
        $main.append($("<table>").addClass("logs").append($tbody));
          ...
          for (var index in logs) {
            ...
            $tbody.append(
              $("<tr>")
                .html(
                  $("<td>")
                    .text(UI.format.dateTime(logs[index][0], "long"))
                    .css("white-space", "nowrap")
                )
                .append(
                  $("<td>")
                    .html(color(logs[index][1]))
                    .css("text-align", "center")
                )
                .append($("<td>").html($content).css("text-align", "left"))
            );
          }
        }
        buildLogsTable();
      ...
      case "Statistics":
        ...
        var graphs = mist.stored.get().graphs
          ? $.extend(true, {}, mist.stored.get().graphs)
          : {};
        ...
        mist.send(
          function () {
            ...
            for (var i in mist.data.capabilities.cpu) {
              UI.plot.datatype.templates.cpuload.cores +=
                mist.data.capabilities.cpu[i].cores;
            }
                ...
                $("<h3>").text("Select the data to display"),
                {
                  ...
                  function: function () {
                    ...
                    if ($(this).val() == "new") {
                      ...
                      $id
                        .setval("Graph " + (Object.keys(graphs).length + 1))
                        .closest("label")
                        .show();
                    } else {
                      ...
                      $s.children("option")
                        .prop("disabled", true)
                        .filter('[value="' + xaxistype + '"]')
                        .prop("disabled", false);
                      $id.closest("label").hide();
                    }
                    if (
                      $s.children('option[value="' + $s.val() + '"]:disabled')
                        .length
                    ) {
                    ...
                    switch ($(this).getval()) {
                      ...
                      case "memload":
                        $s.find("input[type=radio]")
                          .not('[value="total"]')
                          .prop("disabled", true);
                        $s.find('input[type=radio][value="total"]').prop(
                          "checked",
                          true
                        );
                        break;
                  ...
                  buttons: [
                    {
                      ...
                      function: function () {
                        ...
                        if (saveas.graph == "new") {
                          ...
                          $UI
                            .find("select.graph_ids")
                            .append($("<option>").text(graph.id))
                            .val(graph.id)
                            .trigger("change");
                        } else {
            ...
            for (var i in graphs) {
              ...
              $graph_ids.append($("<option>").text(graph.id)).val(graph.id);
            ...
            content: [
              {
                ...
                body: [
                  UI.format.bytes(mem.used * 1024 * 1024) +
                    " (" +
                    UI.format.addUnit(load.memory, "%") +
                    ")",
            ...
            content: [
              {
                ...
                body: [
                  UI.format.addUnit(
                    UI.format.number(mist.data.capabilities.cpu_use / 10),
                    "%"
                  ),
        ...
        $main
          ...
          .append(
            $("<table>")
              ...
              .append(
                $("<tr>")
                  .append($("<td>").append($memory))
                  .append($("<td>").append($loading))
              )
              .append(
                $("<tr>").append($("<td>").append($cpu).attr("colspan", 2))
              )
          );
      ...
      case "Email for Help":
        ...
        config =
          "Version: " + mist.data.config.version + "\n\nConfig:\n" + config;
...
var mist = {
      ...
      error: function (jqXHR, textStatus, errorThrown) {
        ...
        if (!opts.hide) {
          ...
          $("#message")
            .addClass("red")
            .text(
              "An error occurred while attempting to communicate with MistServer:"
            )
            .append($("<br>"))
            .append($("<span>").text(textStatus))
            .append(
              $("<a>")
                .text("Send server request again")
                .click(function () {
        ...
        switch (d.authorize.status) {
          case "OK":
            ...
            var keep = [
              ...
              "totals",
              "bandwidth",
            ]; //streams was already copied above
            for (var i in save) {
              if (keep.indexOf(i) == -1) {
                delete save[i];
              }
            ...
            if ("totals" in d) {
                  ...
                  for (var i in main.data) {
                    ...
                    if (i == 0) {
                      ...
                      if (main.interval[interval_n][0] <= 0) {
                        ...
                        if (interval_n < main.interval.length - 1) {
                          insert += 2;
                        }
                      }
                    }
              ...
              if ("fields" in d.totals) {
                ...
                savereadable(
                  sendData.totals.streams,
                  sendData.totals.protocols,
                  d.totals
                );
              } else {
                for (var i in d.totals) {
                  savereadable(
                    sendData.totals[i].streams,
                    sendData.totals[i].protocols,
                    d.totals[i]
                  );
                }
              }
  ...
  inputMatch: function (match, string) {
    ...
    for (var s in match) {
      ...
      var regex = new RegExp(
        "^(?:[a-zA-Z]:)?" + query + "(?:\\?[^\\?]*)?$",
        "i"
      ); //case insensitive, and ignore everything after the last ?
  ...
  convertBuildOptions: function (input, saveas) {
    ...
    function processEle(j, i, ele) {
      ...
      if ("type" in ele) {
        switch (ele.type) {
          ...
          case "uint":
            ...
            if ("min" in ele) {
              obj.min = Math.max(obj.min, ele.min);
            }
            break;
          case "json":
          case "debug":
          ...
          case "select":
            ...
            if (obj.validate.indexOf("required") >= 0) {
              obj.select.unshift([
                "",
                "placeholder" in obj ? "Default (" + obj.placeholder + ")" : "",
              ]);
            }
            break;
          case "sublist": {
      ...
      if ("influences" in ele) {
        obj["function"] = function () {
          ...
          $(".UIelement:not(:visible) style.dependencies:not(.hidden)").each(
            function () {
    ...
    for (var j in type) {
      if (input[type[j]]) {
        build.push($("<h4>").text(UI.format.capital(type[j]) + " parameters"));
        var list = Object.keys(input[type[j]]); //array of the field names
        if ("sort" in input) {
          //sort by key input.sort
          list.sort(function (a, b) {
            return ("" + input[type[j]][a][input.sort]).localeCompare(
              input[type[j]][b][input.sort]
            );
          });
        }
  ...
  stored: {
    ...
    set: function (name, val) {
      ...
      settings[name] = val;
      mist.send(function () {}, { ui_settings: settings });
    },
    del: function (name) {
  ...
  if (opts && "type" in opts) {
    ...
    switch (
      ...
      case "checklist":
        val = [];
        $(this)
          .find(".checklist input[type=checkbox]:checked")
          .each(function () {
  ...
  if (opts && "type" in opts) {
    ...
    switch (
      ...
      case "hostlimited":
        ...
        for (var i in val) {
          subUI.values.append(subUI.prototype.clone(true).val(val[i]));
        }
        subUI.blackwhite.trigger("change");
        break;
      case "radioselect":
        ...
        var $l = $(this)
          .find('label > input[type=radio][value="' + val[0] + '"]')
          .prop("checked", true)
          .parent();
        if (val.length > 1) {
      ...
      case "checklist":
        var $inputs = $(this)
          .find(".checklist input[type=checkbox]")
          .prop("checked", false);
        for (i in val) {
      ...
      case "unix":
        if (typeof val != "undefined") {
          ...
          ); //correct for the browser being a pain and converting to UTC
      ...
      case "inputlist":
        ...
        for (var i in val) {
          $(this).append($(this).data("newitem")().val(val[i]));
        }
                ...
                .append(
                  $("<button>")
                    ...
                    .click(function () {
                      ...
                      $field.data("build")(
                        Object.assign({}, $field.getval()[index]),
                        index
                      );
                    })
                )
...
function triggerRewrite(trigger) {
  if (typeof trigger == "object" && typeof trigger.length == "undefined") {
    return trigger;
  }
  return (obj = {
    handler: trigger[0],
